      ...
      >
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-[#6B7280]">{expanded ? "Ocultar" : "Configurar"}</span>
            <svg
      ...
      {expanded && (
        <CardContent className="pt-0 space-y-5">
          ...
          <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
            <svg className="w-4 h-4 text-[#D97706] mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {loadError && (
            <div className="flex items-center justify-between bg-red-50 border border-red-200 rounded-xl px-4 py-3">
              <p className="text-xs text-[#C0392B]">{loadError}</p>
              <button
                onClick={loadAccounts}
                className="text-xs text-[#C6A15B] hover:text-[#B8904A] underline underline-offset-2 transition-colors ml-4 shrink-0"
              >
                Reintentar
              </button>
            </div>
...
function AccountGroup({
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-widest">{title}</p>
      <div className="border border-[#E5DED2] rounded-xl overflow-hidden divide-y divide-[#E5DED2]">
        {accounts.map((a) => (
          <AccountRow
            key={a.account}
            account={a}
...
function AccountRow({
  ...
  return (
    <div className="px-4 py-4 bg-white hover:bg-[#FDFAF7] transition-colors">
      {/* Account header */}
      <div className="flex items-center gap-2.5 mb-3">
        <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: color }} />
        <span className="text-sm font-semibold text-[#111827]">{account.account}</span>
        <span className={cn(
          "text-xs px-2 py-0.5 rounded-full font-medium border",
          account.isCredit
            ? "bg-purple-50 text-purple-700 border-purple-200"
            : "bg-[#F8F5F0] text-[#6B7280] border-[#E5DED2]"
        )}>
      ...
      <div className="flex flex-wrap gap-x-5 gap-y-1.5 mb-3 text-xs">
        <span className="text-[#6B7280]">
          Calculado:{" "}
          <span className="text-[#4B5563] tabular-nums">{formatMXN(account.calculatedBalance)}</span>
        </span>
        {hasAdjustment && (
          <span className="text-[#6B7280]">
            Ajuste:{" "}
            <span className={cn(
              ...
              account.balanceAdjustment >= 0 ? "text-[#16A34A]" : "text-[#C0392B]"
            )}>
        ...
        <span className="text-[#6B7280]">
          Mostrado:{" "}
          <span className="text-[#111827] font-semibold tabular-nums">{formatMXN(account.currentBalance)}</span>
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-2.5">
        <div className="flex flex-col gap-1.5">
          <label className="text-xs text-[#6B7280]">Saldo actual real (MXN)</label>
          <div className="relative">
            ...
            <input
              ...
              className={cn(
                "w-44 pl-7 pr-3 py-2 text-sm rounded-xl bg-white text-[#111827] tabular-nums",
                "focus:outline-none focus:ring-1 focus:ring-[#C6A15B]/50 transition-all",
                row.error
                  ? "border border-red-300"
                  : "border border-[#E5DED2] hover:border-[#D4CCBE]"
              )}
              placeholder="0.00"
            />
          </div>
      ...
      <div className="mt-2.5 min-h-[16px]">
        {row.error && (
          <p className="text-xs text-[#C0392B]">✗ {row.error}</p>
        )}
